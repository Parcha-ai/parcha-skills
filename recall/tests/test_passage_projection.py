...
class PassageProjectionTests(unittest.TestCase):
    ...
    def test_need_pages_keep_each_need_independent(self) -> None:
        def candidate(document: int) -> dict:
            return {
                "source_id": "source:test",
                "logical_document_id": f"ldoc_{document:032x}",
        ...
        rankings = {
            "need-a": [candidate(index) for index in range(1, 7)],
            "need-b": [candidate(index) for index in range(101, 107)],
        }

        def response(query, **_kwargs):
            rows = rankings[query]
            return {
                "results": rows,
                "arms": {
                    "dense": rows,
                    "passage-lexical": rows,
                    "sparse-exact": rows,
                },
                "diagnostics": {
                    "dense_status": "ok",
        ...
        self.assertEqual(
            [item["need_index"] for item in result["results"]],
            [0, 0, 1, 1],
        )
        self.assertEqual(
            result["diagnostics"]["need_pages"],
            [
                {
                    "need_index": 0,
                    "page": 1,
