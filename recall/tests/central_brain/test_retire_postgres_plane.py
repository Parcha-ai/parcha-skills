...
class MigrateRunnerTest(unittest.TestCase):
    ...
    def test_mandatory_versions_rerun_idempotently_and_067_is_deferred_by_default(self) -> None:
        ...
        self.assertEqual(_migration_files_applied(connection), [v for v in expected_schema_versions() if v != 67])
        self.assertEqual(result["applied"], [v for v in expected_schema_versions() if v >= 66 and v != 67])
        ...
        self.assertEqual(result["applied"], [v for v in expected_schema_versions() if v >= 68])
        self.assertEqual(result["deferred"], [67])
    ...
    def test_fresh_database_applies_every_mandatory_version_in_order(self) -> None:
        ...
        result = self._migrate(_store("postgres"), connection)
        self.assertEqual(_migration_files_applied(connection), [v for v in expected_schema_versions() if v != 67])
        self.assertEqual(result["applied"], [v for v in expected_schema_versions() if v != 67])
        self.assertEqual(result["deferred"], [67])
    ...
    def test_retirement_applies_once_on_the_turbopuffer_plane(self) -> None:
        ...
        tail = [v for v in expected_schema_versions() if v >= 67]
        self.assertEqual(_migration_files_applied(connection)[-len(tail):], tail)
        connection.executed.clear()
        self.assertEqual(result["applied"], tail)
        ...
        self.assertEqual(again["skipped"], len(expected_schema_versions()))
        self.assertEqual(_migration_files_applied(connection), [])
        plain = self._migrate(store, connection)
        self.assertEqual((plain["applied"], plain["skipped"], plain["deferred"]), ([], len(expected_schema_versions()), []))
