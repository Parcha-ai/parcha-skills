                ...
                f"""WITH batch AS MATERIALIZED (
                     SELECT event.tenant_id,event.source_id,event.event_id
                      ...
                      WHERE event.body_location='inline'
                        ...
                        AND EXISTS (
                            SELECT 1
                              FROM canonical_evidence_documents AS evidence
                             WHERE evidence.tenant_id=event.tenant_id
                               AND evidence.source_id=event.source_id
                               AND evidence.native_parent_id=COALESCE(
                                   event.native_parent_id,event.native_id
...
def main() -> None:
    ...
    retire_chunk_vector = sub.add_parser("storage-retire-chunk-search-vector",
        help="preview the unused generated chunk vector retirement; requires turbopuffer, 067 and no chunk GIN")
    retire_chunk_vector.add_argument("--apply", action="store_true",
        help="drop only the derived column; physical reclamation needs a separate rewrite")
    ...
    elif args.command == "storage-retire-chunk-search-vector":
        print(json.dumps(retire_chunk_search_vector(store, apply=args.apply), sort_keys=True))
