...
def main() -> None:
    ...
    parser.add_argument(
        "--interval",
        type=float,
        default=float(os.environ.get("RECALL_INTERVAL_SECONDS", "30")),
    )
    parser.add_argument(
        "--max-scan-records",
        type=int,
        default=int(os.environ.get("RECALL_MAX_SCAN_RECORDS", "1000")),
    )
    parser.add_argument(
        "--max-scan-seconds",
        type=float,
        default=float(os.environ.get("RECALL_MAX_SCAN_SECONDS", "20")),
    )
    parser.add_argument(
        "--archive-workers",
        type=int,
    ...
    parser.add_argument(
        "--bulk-manifest-archive",
        action="store_true",
        default=os.environ.get("RECALL_BULK_MANIFEST_ARCHIVE") == "1",
    )
    parser.add_argument(
        "--bulk-bundle-records",
        type=int,
