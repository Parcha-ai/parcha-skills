...
SKILLS = ("hands-free", "parable", "cascade", "recall", "recap", "tether", "desloppify", "autoqa", "precap")
...
class PortablePackagingTest(unittest.TestCase):
    ...
    def test_effective_comms_is_a_complete_managed_snippet(self):
        ...
        self.assertLessEqual(len(snippet.split()), 135)
        self.assertIn("ISO 24495-1:2023 (relevant, findable, understandable, usable)", snippet)
        self.assertIn("W3C COGA (clear content, focus recovery, no reliance on memory)", snippet)
        self.assertIn("US Plain Writing Act and NARA's ten principles", snippet)
        self.assertIn("JAN guidance on written instructions, checklists, and task separation", snippet)
        self.assertIn("Write like a human talking to another human", snippet)
        self.assertIn("Lead with the answer, result, or next action", snippet)
        self.assertIn("marketing rhetoric", snippet)
        self.assertIn("Be brief, not incomplete", snippet)
        self.assertIn("Preserve accuracy and required safety, security, privacy, legal caveats", snippet)
        self.assertIn("does not claim certification or", readme)
        self.assertIn("[`bro`](https://github.com/plannotator/dev-skills", readme)
