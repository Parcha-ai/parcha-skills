...
class PortablePackagingTest(unittest.TestCase):
    ...
    def test_effective_comms_is_a_complete_managed_snippet(self):
        ...
        self.assertIn("**Relevant:**", snippet)
        self.assertIn("**Findable:**", snippet)
        self.assertIn("**Understandable:**", snippet)
        self.assertIn("**Usable:**", snippet)
        self.assertIn("Lead with the answer, result, or next action", snippet)
        self.assertIn("No marketing rhetoric in technical writing", snippet)
