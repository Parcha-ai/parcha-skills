...
| H2-h temporal hints (soft window boost + windowed dense pass) | doing | agent h2-h | target: "around May 2-4" questions (f50dc1c15) |
...
| H2-l card saves per-candidate fused/rerank/blended (`rerank_evidence`) + `evals/rerank_blend_replay.py` offline blend grid | #540 (CI red, investigating), #541 merged | lead | lets RECALL_RERANK_BLEND be tuned from one card run instead of a deploy per value |
