...
class SchemaRehearsalTest(unittest.TestCase):
    ...
    def test_crashed_rehearsal_leaves_the_predecessor_bootable(self):
        """A rehearsal never touches the live database, so a crashed one
        must not keep the deployed broker from starting. Bricking production
        with an evidence-only run is worse than the risk it guards."""
        paths = self.build_environment()
        ...
        recovery = self.module.classify_recovery(paths["state"])
        self.assertEqual(recovery["state"], "incomplete_backup_verified")

        with mock.patch.dict(os.environ, {
            "HOME": str(paths["state"].parents[1]),
            "XDG_STATE_HOME": str(paths["state"].parent),
        }, clear=False):
            # No exception: the live database was never mutated.
            store, lock_fd = self.runtime.open_locked_store(paths["database"])
            del store
            os.close(lock_fd)

    def test_live_mutating_operation_still_blocks_startup(self):
        """The gate must still fail closed for anything that may have
        written to the live database."""
        paths = self.build_environment(name="live-op")
        receipt_file = paths["state"] / "schema" / "active.json"
        ...
        self.receipts.advance(
            receipt_file, expect=receipt, to_phase="db_committed"
        )
