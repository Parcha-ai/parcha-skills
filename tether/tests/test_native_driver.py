...
class NativeDriverTest(unittest.TestCase):
    ...
    def test_recovery_after_unobserved_exit_is_uncertain_not_success(self):
        attempt = self.schedule()
        launched = self.driver.launch(attempt, **self.shell("printf 'sobrevivio'"))
        launched["process"].wait(timeout=30)
        # A driver that crashed after acceptance never reaped this process,
        # so its exit status is unrecoverable. A complete-looking response
        # file is indistinguishable from a truncated one written just before
        # an OOM kill, so recovery must not claim success.
        recovery = self.driver.recover(attempt)
        self.assertEqual(recovery["classification"], "uncertain")
        self.assertEqual(recovery["state"], "uncertain")
        self.assert_invariants_hold()
...
class UnobservedExitTest(NativeDriverTest):
    """A crash we did not reap is never laundered into a success."""

    def test_partial_output_after_unobserved_exit_is_uncertain(self):
        attempt = self.schedule()
        launched = self.driver.launch(
            attempt,
            **self.shell("printf 'partial answer'; sleep 30"),
        )
        # Kill the child the way an OOM killer would, without reaping it
        # through this driver: the exit status is gone.
        os.kill(launched["process"].pid, 9)
        launched["process"].wait(timeout=30)
        ...
        status = self.runtime.attempt_status(attempt["attempt_id"])
        self.assertEqual(status["state"], "uncertain")
        self.assertEqual(status["error_code"], "exit_status_unobserved")
        # The lease stays open so the blocker is operator-visible, and the
        # partial text is never published as the answer.
        self.assertTrue(status["lease_open"])
        connection = sqlite3.connect(self.db_path)
        try:
            response = connection.execute(
                "SELECT response_ref FROM native_attempts WHERE attempt_id=?",
                (attempt["attempt_id"],),
            ).fetchone()[0]
        finally:
            connection.close()
    ...
    def test_second_recovery_pass_stays_uncertain_and_never_respawns(self):
        attempt = self.schedule()
        launched = self.driver.launch(attempt, **self.shell("sleep 30"))
        os.kill(launched["process"].pid, 9)
        launched["process"].wait(timeout=30)

        self.driver.recover(attempt)
        again = self.driver.recover(attempt)

        self.assertEqual(again["classification"], "uncertain")
        with self.assertRaises(self.driver_module.NativeDriverError):
            self.driver.launch(attempt, **self.shell("printf 'replay'"))
