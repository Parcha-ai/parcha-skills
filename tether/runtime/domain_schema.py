...
PRESERVED_MANIFEST_KEYS = (
    ...
    "egress_receipts",
    # Liveness of each binding, in a form comparable across both schemas.
    # Without it, a round trip that quietly closes every binding passes the
    # preservation check while leaving users' threads dead. (`binding_records`
    # cannot serve here: the v17 and v18 digests cover different columns.)
    "binding_liveness",
)
...
def logical_manifest_v17(connection: sqlite3.Connection) -> dict[str, Any]:
    ...
    return {
        ...
        "binding_liveness": _digest_rows(
            connection.execute(
                """
                SELECT bridge_id,
                       CASE WHEN status='closed' THEN 'not_live' ELSE 'live' END
                FROM bridges ORDER BY bridge_id
                """
            )
        ),
...
def logical_manifest_v18(connection: sqlite3.Connection) -> dict[str, Any]:
    return {
        ...
        # Same shape as the v17 digest: identity plus is-it-live. Both
        # schemas agree that only closure kills a binding — 'pending' /
        # 'rebind_required' are documented demotions that keep the thread
        # bound and are therefore still live.
        "binding_liveness": _digest_rows(
            connection.execute(
                """
                SELECT binding_id,
                       CASE WHEN state='closed' THEN 'not_live' ELSE 'live' END
                FROM thread_bindings ORDER BY binding_id
                """
            )
        ),
        "binding_records": _digest_rows(
...
def backup_database(source_path: Path, destination_path: Path) -> None:
    ...
    try:
        # sqlite3 connections as context managers commit but never close;
        # this is the durability path, so close both explicitly.
        with source_db, backup:
