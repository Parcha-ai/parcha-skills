...
class NativeDriver:
    ...
    def recover(self, attempt: dict[str, Any]) -> dict[str, Any]:
        ...
        if phase == "accepted":
            ...
            # The exact process exited while we were away. We reaped nothing,
            # so its exit status is unrecoverable: a clean finish and an
            # OOM kill are indistinguishable from here, and a partial
            # response file looks exactly like a complete one. Claiming
            # success would launder a crash into a committed answer, so the
            # attempt becomes operator-visible `uncertain` and is never
            # re-executed. Only a driver that observed the exit itself may
            # emit a terminal receipt.
            # Emit it as a real driver receipt, not a bare state write: the
            # schema requires the attempt's last receipt to agree with its
            # state, and this observation ("the process is gone and I never
            # saw its status") is exactly what a receipt is for.
            self._emit(
                attempt,
                state="uncertain",
                error_code="exit_status_unobserved",
                incarnation=f"recovery:{pid}:{recorded_start or 'unknown'}",
            )
            self._write_journal(attempt_id, {
                "phase": "uncertain",
                "request_id": attempt["driver_request_id"],
                "lease_fence": attempt["lease_fence"],
                "pid": pid,
                "starttime": recorded_start,
            })
            return {"classification": "uncertain", "state": "uncertain"}
        if phase == "uncertain":
            # Already classified; a second pass never re-executes or guesses.
            return {"classification": "uncertain", "state": status["state"]}
        if phase == "terminal":
            return {"classification": "already_terminal", "state": status["state"]}
        raise NativeDriverError("journal_corrupt")
