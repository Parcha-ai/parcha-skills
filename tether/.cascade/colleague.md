...
| C6 | Move 2 design note: stream-json driver per binding, hooks → broker, what deletes | C2 | note in tether/docs/plans/ with line-count ledger and test plan | todo | Prototype 2026-09-06 01:1x UTC on greppy3: one `claude -p --resume <sid> --input-format stream-json --output-format stream-json` process took two user turns; both results carried the same session id, turn 2 recalled turn 1 ("TWO"), turn 2 latency 1.4 s, exit 0. Session id parse + stream events confirmed. Codex: `codex app-server` over stdio answered initialize and thread/list (cwd filter, previews) in <2 s; turn/start on a live thread is the next probe |
...
- Next: C5 fleet roll with scratchpad `hermes-fleet-upgrade.sh` (primary boxes), by-hand for the /opt runtime of mikael/irma; C4 needs 48 h of new #agent-hub traffic after C3 (weekend was quiet: 3 turns).
