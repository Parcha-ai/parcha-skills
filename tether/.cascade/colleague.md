...
| C7 | Session driver (store.py + session_driver.py) behind `driver = session`, tests | C6 | 230+ tests green; bound turn on claudio via new driver with same evidence quality | doing | PR 458 open 2026-09-08: store (4 tables, same method/refusal contract), driver (one stream-json process per binding, idle sweep, crash/timeout/error → failure notice, trailing NO_REPLY = silence), 257 tests incl. fake-harness suite. Codex stays per-turn `exec resume` in v1. Canary: claudio `driver = session` after merge |
...
- Next: merge PR 458 → claudio canary on `driver = session` → live smoke (bound turn, peer turn, close) → fleet flip → C8 deletion. C4 after 48 h of traffic (from 2026-09-08). Also: docs/plans + scripts/fleet now live on main via PR 458; this branch keeps only .cascade/.
